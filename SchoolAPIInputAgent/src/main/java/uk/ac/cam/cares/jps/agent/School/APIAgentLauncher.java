...
public class APIAgentLauncher extends JPSAgent
{
    ...
    private static final String AGENT_ERROR_MSG = "The SchoolAPI input agent could not be constructed!";
    ...
    public static JSONObject initializeAgent(String []args)
    {
        ...
        JSONObject schoolReadings;
        JSONObject programmeReadings; 
        JSONObject ccaReadings;

        try
        {
            schoolReadings = connector.getGeneralReadings();
        }
        catch(Exception e)
        {
        ...
        Log.info(String.format("Retrieved %d school readings", schoolReadings.getJSONObject("result").getJSONArray("records").length()));
        jsonMessage.accumulate("Result","Retrieved"+schoolReadings.getJSONObject("result").getJSONArray("records").length()+" school readings");

        // if(!schoolReadings.isEmpty())
        // {
        ...
        Log.info(String.format("Retrieved programmes readings for %d schools", programmeReadings.getJSONObject("result").getJSONArray("records").length()));
        jsonMessage.accumulate("Result","Retrieved"+programmeReadings.getJSONObject("result").getJSONArray("records").length()+"programme readings");
        ...
        //To complete according to API Query Builder 
        ...
        try
        {
            queryBuilder.instantiateIfNotInstantiated(schoolReadings,programmeReadings,ccaReadings);
            Log.info("All Data IRIs within School Readings successfully instantiated");
